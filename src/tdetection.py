...
def run_tamper_detection(file_path, output_dir=None):
    ...
    with open(output_csv, "w", newline="", encoding="utf-8") as log_file:
        ...
        csv_writer.writerow(["frame", "timestamp", "event", "laplacian_variance", "brightness_ratio", "note"])
        ...
        try:
            while True:
                ...
                if frame_idx % frame_interval == 0:
                    ...
                    # Set baseline brightness from first frame
                    ...
                    if cover_counter >= persistence_frames and not cover_active:
                        cover_active = True
                        event = "COVER_START"
                        note = "Tampering detected - camera covered/blocked"
                        ts = frame_idx / fps
                    ...
                    # Detect cover end
                        ...
                        cover_active = False
                        event = "COVER_END"
                        note = "Tampering ended - camera uncovered"
                        ts = frame_idx / fps
                        csv_writer.writerow([
                            frame_idx, 
                            format_timestamp(ts), 
